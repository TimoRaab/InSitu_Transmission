...
% figs2keep = [13670 13671 13672];
% all_figs = findobj(0, 'type', 'figure');
% delete(setdiff(all_figs, figs2keep))
%%
MeasCounter = 0;
basedir = 'D:\InSitu\SpinCoating\221006'; % Dateipfad zu ordner
% refBasedir = 'C:\Measurements\Measurements\InSitu\SpinCoating\211115';
refBasedir = basedir;
refName = '20221006_124219_TR_5PerCNTest_Blend_C19_meas.spin';
fname = '20221006_125528_TR_5PerCNTest_PDINN_C19_meas.spin';
cellName = 'C30';
titleNameStart = 'Direct - GlasRef';
fNameSaveStartTemp = 'Direct_PDINN_GlasRef';
dirNameSave = basedir;
savePic = 0;
...
tempPos = regexp(refName,'_');
tempFname = refName(tempPos(2)+1:end);
refName = replaceBetween(refName, tempPos(end-1)+1, tempPos(end)-1, cellName);
% titleName = [cellName ' - ' titleNameStart];
% fNameSaveStart = fNameSaveStartTemp;
% fNameSaveStart = [cellName '_' fNameSaveStart];
...
%% Correct for refName date
correctedPath = correctPathForDate(fullfile(refBasedir, refName));
splitter = regexp(correctedPath,filesep);

% basedir = pathstring(1:splitter(end));
refName = correctedPath(splitter(end)+1:end);
disp(['refName: ' refName])
...
%%
%%
[spectra, time, wl, ref, dark, startTime] = readSpinCoater(fullfile(refBasedir,refName));  %liest die Spektren ein
...
%% Import Data
[spectra, time, wl, ~, dark, startTime] = readSpinCoater(fullfile(basedir,fname));  %liest die Spektren ein
spectra = squeeze(spectra); % entfernt eine Dimension
dark = squeeze(dark);
% ref = squeeze(ref);
[~,indWlMin] = min(abs(wl-min(wlRange))); % findet die Indizes f�rs plotten
...
for codeFolding = 1
    ...
    timeRangeDetectionThickness = (speedValueTimes(2)+1):0.05:(timeStamps(2)-1);
