...
%%
MeasCounter = 0;
basedir = 'C:\Users\Cerberus\Desktop\Measurements\210714'; % Dateipfad zu ordner
fname = '20210714_112511_TR_Y6PM6_Additive_B30_meas.spin';
wlRange = [420e-9 700e-9]; % gibt den Wellenbereich zum plotten an
timeRange = [-120 180]; % gibt den Zeitbereich zum plotten an
wlPlot = [446 509 558 605]*1e-9; % f???r den plot einer Wellenl???nge ???ber die Zeit
timePlot = [1:5:20];

timeRangeFFT = [40 100]; %Zeitbereich f???r die FFT (hier soll das Spektrum konstant sein)
...
if (createReferenceSpectrum)
    fname_set = [fname(1:regexp(fname, '_meas.spin')) 'comments.txt'];
...
for i = 1:length(wlPlot)
    ...
    ylabel('Transmittance')
    figure(13000+MeasCounter)
    plot(time(indTimeMin:indTimeMax), M(indWL, indTimeMin:indTimeMax), 'Linewidth', 2);
    hold on
end
