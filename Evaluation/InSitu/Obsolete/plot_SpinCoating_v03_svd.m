...
%%
MeasCounter = 0;
basedir = 'E:\Measurements\InSitu\SpinCoater\210628'; % Dateipfad zu ordner
fname = '20210628_134809_TR_Y6PM6_CF_Reference_A31_meas.spin';
...
% speedRangeFFT = [1000 2000]/60; % Bereich der Rotationsgeschwindigkeit, in welcher gesucht wird
...
disp(['Speed: ' num2str(xDat(ind)*60) 'rpm'])


%% crop spectra in time
[~,indTemp] = min(abs(time + 1));
time = time(indTemp:end);
trans = trans(:, indTemp:end);
...
C{k} = S(k,k) .* U(:,k) .* V(:,k)';
figure;% Cx(k) = axes;
% surf(Cx(k),time,wl,C{k},'LineStyle','none', 'FaceColor','interp');
imagesc(time, wl, C{k});
% view(2)
% rotate3d on
title(num2str(k));
